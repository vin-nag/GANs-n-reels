...
import random
from music21 import *
from src.Generation.Cleaning.Stats import StatsHandler

STYLETIMES = {'hornpipe': '4/4',
              'slip jig': '9/8',
              'mazurka': '3/4',
              'three-two': '3/2',
              'waltz': '3/4',
...
class RandomGenerator:
    ...
    def __init__(self, option, filename, base_key):
        self.stats = StatsHandler(filename, base_key)
        self.songs = {}
        self.option = option
    ...
    def randStyle(self):
        out = []
        styles = self.stats.get_styles()
        if self.option == 0:
            for x in styles: out += [x]
            out = random.choice(out)
        else:
            for x in styles: out += [x] * styles[x]
            out = random.choice(out)
        return out
    ...
    def randTime(self):
        out = []
        meters = self.stats.get_meters()
        if self.option == 0:
            for x in meters: out += [x]
            out = random.choice(out)
        elif self.option == 1:
            for x in meters: out += [x] * meters[x]
            out = random.choice(out)
        else:
            out = STYLETIMES.get(style, default='4/4')
        return out

    # Randomly chose a key
    def randMode(self):
        out = []
        modes = self.stats.get_modes()
        if self.option == 0:
            for x in modes: out += [x]
            out = random.choice(out)
        else:
            for x in modes: out += [x] * modes[x]
            out = random.choice(out)
        return out
...
if __name__ == '__main__':
    ...
    print(song)
    player = Decoder.from_dict(self.songs)
