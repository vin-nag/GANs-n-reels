...
class Decoder:
    ...
    def test_play(self, num=None, save=False):
        if num is None: num = random.randint(0, len(self.tunes)-1)
        self.play(num)
        if not save: save = input('Would you like to save the song as a MIDI file? Y/N:   ').upper() == 'Y'
        if save: self.save_tune(num)

    def test_play_all(self):
        for x in range(len(self.tunes)): self.test_play(x)
